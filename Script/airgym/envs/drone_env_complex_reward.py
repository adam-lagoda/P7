    ...
    def _compute_reward(self):
        ...
        if self.state["collision"]:
            ...
            if self.cam_coords["xmin"]==0 and self.cam_coords["ymin"]==0 and self.cam_coords["xmax"]==self.cam_coords["width"] and self.cam_coords["ymax"]==self.cam_coords["height"]:
                print("Agent update - view optimized")
                reward +=1000
                done = 1
                episode_length = 0
            # Object's center in a 135x135px box
            if abs(x_cam_middle - x_obj_middle) < (0.07*self.cam_coords["width"]): 
                print("Agent update - object in bounds of X")
                reward += 30
            else:
                print("Agent update - object outside of bounds of X")
                reward -= 1
            if abs(y_cam_middle - y_obj_middle) < (0.12*self.cam_coords["height"]):
                print("Agent update - object in bounds of Y")
                reward += 30
            else:
                print("Agent update - object outside of bounds of Y")
                reward -=1
