...
class AirSimDroneEnv(AirSimEnv):
    ...
    def reward_x(self, x_center, width, limit):
        if x_center >= 0 and x_center < (width/2-limit):
            reward = ((1/(width/2-limit)) * x_center) - 1
        elif x_center >= (width/2-limit) and x_center <= (width/2+limit):
            reward = -(1/limit)*abs(x_center-(width/2)) + 1 
        elif x_center > (width/2+limit) and x_center <= width:
            reward = -(1/(width/2-limit))*(x_center-(width/2+limit)) 
        else:
            reward = -1
        return reward
    ...
    def _compute_reward(self):
        ...
        if self.state["collision"]:
            ...
            '''
            reward_x_obj = self.reward_x(self.x_obj_middle, self.cam_coords["width"], 400)
            #print("X_Object - ", self.x_obj_middle)
            #print("Width - ", self.cam_coords["width"])
            print("Agent update - centering reward = ", reward_x_obj)
            reward += reward_x_obj
