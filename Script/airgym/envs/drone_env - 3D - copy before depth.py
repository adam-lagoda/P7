...
class AirSimDroneEnv(AirSimEnv):
    def __init__(self, ip_address, step_length, image_shape):
        ...
        self.action_space = spaces.Discrete(6) # Number of possible actions/movements in the action_space
        self._setup_flight()
        ''' DEBUGGING
    ...
    def _compute_reward(self):
        ...
        if self.state["collision"]:
            ...
            '''
            reward_x_obj = self.reward_center(self.x_obj_middle, self.cam_coords["width"], 400)
            reward_y_obj = self.reward_center(self.y_obj_middle, self.cam_coords["height"], 400)
            #print("X_Object - ", self.x_obj_middle)
            #print("Width - ", self.cam_coords["width"])
            print("Agent update - centering x reward = ", reward_x_obj)
            print("Agent update - centering y reward = ", reward_y_obj)
            reward += reward_x_obj
            reward += reward_y_obj
